...
class ProductsController extends Controller
{
    ...
    { // using the Dynamic Routes with the foreach loop
        ...
        if ($request->ajax()) {
            ...
            if (isset($_REQUEST['search']) && !empty($_REQUEST['search'])) { // If the Search Form is used, handle the Search Form submission
                // New Catering Setup    // Check front/layout/header.blade.php
                if ($_REQUEST['search'] == 'new-arrivals') {
                    ...
                    $categoryDetails['breadcrumbs'] = 'New Catering Setups';
                    $categoryDetails['categoryDetails']['category_name'] = 'New Catering Setups';
                    $categoryDetails['categoryDetails']['description'] = 'New Catering Setups';
                    ...
                    $categoryProducts = \App\Models\Product::select(
                        ...
                        )->where('products.status', 1)->orderBy('id', 'Desc'); // Show from the latest to the earliest (New Catering Setup!)
                    // dd($categoryProducts);

                    // Best Setups    // Check front/layout/header.blade.php
                } elseif ($_REQUEST['search'] == 'best-sellers') {
                    $search_product = $_REQUEST['search'];

                    // We fill in the $categoryDetails array MANUALLY with the same indexes/keys that come from the categoryDetails() method in Category.php model (because in either cases of the if-else statement, we pass in $categoryDetails variable to the view down below)
                    $categoryDetails['breadcrumbs'] = 'Best Setups Products';
                    $categoryDetails['categoryDetails']['category_name'] = 'Best Setups Products';
                    $categoryDetails['categoryDetails']['description'] = 'Best Setups Products';
    ...
    { // Required Parameters: https://laravel.com/docs/9.x/routing#required-parameters
        ...
        $addOns = \App\Models\Addon::where('vendor_id', $vendor_id)->where('categories',  $productDetails['category']['id'])->get(); // sum() the `stock` column of the `products_attributes` table    // sum(): https://laravel.com/docs/9.x/collections#method-sum
        //dd($vendor_id);
    ...
    public function cartAdd(Request $request)
    {
        if ($request->isMethod('post')) { // if the Add to Cart <form> is submitted
            ...
            if ($eventDate > 0) {
                return redirect()->back()->with('error_message', 'This service is not available for this date!');
            } else {
    ...
    public function cart()
    {
        ...
        $meta_title = 'Shopping Cart - Caserole';
    ...
    public function checkout(Request $request)
    {
        ...
        if ($request->isMethod('post')) { // if the <form> in front/products/checkout.blade.php is submitted (the HTML Form that the user submits to submit their Delivery Address and Payment Method)
            ...
            // $order->event_date = $data['payment_gateway'];
            // $order->payment_gateway = $data['payment_gateway'];
            ...
            foreach ($getCartItems as $item) {
                ...
                $cartItem->event_date = $item['event_date'];
                $cartItem->event_time = $item['event_time'];
            ...
            if ($data['payment_gateway'] == 'COD') { // if the `payment_gateway` selected by the user is 'COD' (in front/products/checkout.blade.php), we send the placing the order confirmation email and SMS immediately
                ...
                \Illuminate\Support\Facades\Mail::send('emails.order', $messageData, function ($message) use ($email) { // Sending Mail: https://laravel.com/docs/9.x/mail#sending-mail    // 'emails.order' is the order.blade.php file inside the 'resources/views/emails' folder that will be sent as an email    // We pass in all the variables that order.blade.php will use    // https://www.php.net/manual/en/functions.anonymous.php
                    $message->to($email)->subject('Order Placed - Caserole');
                });

                /*
                // Sending the Order confirmation SMS
                // Send an SMS using an SMS API and cURL
                $message = 'Dear Customer, your order ' . $order_id . ' has been placed successfully with Caserole. We will inform you once your order is shipped';
    ...
    public function checkPincode(Request $request)
    {
        if ($request->ajax()) { // if the request is coming via an AJAX call
            ...
            if ($eventDate > 0) {
                echo 'This service is not available for this date!';
            } else {
                echo 'This service is available for this date.';
            }
        }
    }
